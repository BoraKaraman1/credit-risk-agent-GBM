...
> **TL;DR:** Production-grade credit risk pipeline scoring 2.3M+ Lending Club loans — featuring a medallion data architecture, SHAP-based regulatory-compliant adverse action reasons, automated drift monitoring, and fairness analysis. Training runs in Python (scikit-learn); serving and monitoring run as static Go binaries with pure-Go GBM inference and TreeSHAP. Test AUC of 0.718 on origination-only features (upper end for this dataset, where typical models achieve 0.68–0.73).
...
The system is split across two runtimes: **Python** owns everything that needs the scientific stack (data prep, sklearn training, fairness, Great Expectations, Airflow DAG definitions), while **Go** owns the serving layer (`go/`) — the scoring API, drift/performance monitors, retrain orchestration, and feature-store sync. The trained model is exported to JSON (`pipeline/export_model_json.py`) and the Go services run inference and TreeSHAP natively, with no Python at serving time. Go outputs are verified against the Python stack: predictions to 1e-9 vs sklearn, SHAP values to 1e-6 vs the `shap` library.
...
| Model | scikit-learn HistGradientBoostingClassifier (training) |

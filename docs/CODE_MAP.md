...
- **`dags/credit_risk_pipeline.py`** — Monthly DAG (`credit_risk_pipeline`): bronze_ingest → silver_transform → gold_features → train_challenger → export_model_json → optional reject inference → finalize_challenger → `gbm sync --model challenger`, plus a parallel fairness_analysis task (skips until a champion exists). Sync waits for the branch join, so it always stages the feature contract and score distribution for the final challenger. Promotion to champion stays a human `gbm promote`.
...
| File | What it does |
|---|---|
| `go/main.go` | CLI spine: dispatches `serve` → `inference.Serve()` and the monitoring commands, including explicit `sync --model <champion\|challenger>` and `promote [--offline]` lifecycle modes. Unknown commands print usage, exit 2. |
| `go/inference/server.go` | The HTTP scoring API. Core flow: fetch the applicant snapshot at the model's exact feature version → enforce staleness and schema → predict → calibrate PD/scorecard → decide → produce up to four unique ECOA reason codes → persist the complete audit envelope → record Prometheus business metrics → respond. Enforces the model-card governance gate and fails closed without API keys unless explicitly allowed for development. |
...
| `go/inference/middleware.go` | Middleware chain: request IDs + access logs + HTTP metrics; constant-time API-key auth (`X-API-Key` or Bearer); separate per-client request and scoring token buckets keyed by API key or client IP (`X-Forwarded-For` honored only with `TRUST_PROXY_HEADERS`), with idle-bucket eviction. |
...
| `go/monitoring/promote.go` | `gbm promote`: preflights the running API, idempotently publishes identical retry artifacts under `models/versions/<version>`, atomically swaps the champion pointer, and requires `/reload` to acknowledge the exact version. Activation failure restores and reloads the prior champion. Missing `SCORING_API_URL` fails unless `--offline` is explicitly used while the API is stopped. |
...
| `go/monitoring/sync.go` | `gbm sync --model <champion\|challenger>` validates Gold feature version and ordered columns against the selected model, then bulk-upserts test-set snapshots keyed by `(applicant_id, feature_version)` and stores that model's training score histogram. Champion and challenger feature contracts can coexist through activation. |

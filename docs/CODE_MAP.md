...
- **`dags/credit_risk_monitoring.py`** — Weekly DAG (`credit_risk_monitoring`): outcome backfill → performance + drift monitors in parallel → retrain branch (PSI CRITICAL or AUC drop) invoking `gbm retrain <reason>` → optional advisory LLM review task that always runs and never blocks the loop; a parallel `scoring_log_prune` task enforces audit retention (`gbm prune`). Reports flow via XCom.
...
| `go/monitoring/lock.go` | Advisory flock on `models/.registry.lock` taken by `gbm retrain` and `gbm promote`, so concurrent registry mutations exclude each other instead of relying on Airflow scheduling. |
